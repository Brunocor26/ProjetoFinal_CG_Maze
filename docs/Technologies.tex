...
A biblioteca \textbf{GLM} (OpenGL Mathematics) \cite{GLM} fornece funcionalidades matemáticas essenciais para programação gráfica 3D. GLM implementa tipos e operações compatíveis com GLSL, facilitando a transferência de dados entre CPU e GPU.
...
O \textbf{GLAD} é um carregador de extensões OpenGL que gere a obtenção de ponteiros para funções OpenGL. É necessário porque as funções OpenGL modernas não estão disponíveis diretamente no sistema operativo, devendo ser carregadas em tempo de execução.
